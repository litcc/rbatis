...
impl MssqlConnection {
    /// let cfg = Config::from_jdbc_string(url).map_err(|e|
    /// Error::from(e.to_owned()))?;
...
impl Connection for MssqlConnection {
    fn get_rows(
        ...
        Box::pin(async move {
            ...
            let s =
                v.into_results().await.map_err(|e| Error::from(e.to_string()))?;
            for item in s {
                for r in item {
    ...
    fn ping(&mut self) -> BoxFuture<Result<(), rbdc::Error>> {
        //TODO While 'select 1' can temporarily solve the problem of checking that
        // the connection is valid, it looks ugly.Better replace it with something
        // better way
        Box::pin(async move {
            self.inner
                .as_mut()
