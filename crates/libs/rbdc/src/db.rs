...
/// Represents database driver that can be shared between threads, and can therefore
/// implement a connection pool
pub trait Driver: Debug + Sync + Send {
    fn name(&self) -> &str;
    /// Create a connection to the database. Note that connections are intended to be
    /// used in a single thread since most database connections are not
    /// thread-safe
...
impl Display for ExecResult {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        ...
        f.debug_map()
            ...
            .key(&"last_insert_id")
            .value(&DisplayBox { inner: &self.last_insert_id })
            .finish()
    }
}
...
impl From<(u64, Value)> for ExecResult {
    fn from(value: (u64, Value)) -> Self {
        Self { rows_affected: value.0, last_insert_id: value.1 }
    }
}
...
pub trait Connection: Send {
    ...
    /// but it is recommended to actively close this function so that the database
    /// does not report errors. If &mut self is not satisfied close, when you
    /// need mut self, It is recommended to use Option<DataBaseConnection>
    /// and then call take to take ownership and then if let Some(v) =
    /// self.inner.take() {v.lose ().await; }
