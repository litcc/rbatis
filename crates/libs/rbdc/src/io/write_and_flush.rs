...
// Atomic operation that writes the full buffer to the stream, flushes the stream,
// and then clears the buffer (even if either of the two previous operations failed).
...
impl<S: AsyncWrite + Unpin> Future for WriteAndFlush<'_, S> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        let Self { ref mut stream, ref mut buf } = *self;
        ...
        Pin::new(stream).poll_flush(cx).map_err(|e| Error::from(e.to_string()))
    }
}
