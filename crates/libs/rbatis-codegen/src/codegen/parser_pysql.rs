                ...
                trim_express.starts_with("`") && trim_express.ends_with("`")
            {
                ...
                for mut expr in express {
                    ...
                    if expr.starts_with("start") {
                        ...
                        return Err(Error::from(format!("[rbatis-codegen] express trim node error, for example1  trim 'value':  trim start='value': trim start='value',end='value':   express = {}", trim_express)));
                    }
                }
                return Ok(NodeType::NTrim(TrimNode {
                    childs,
                ...
                return Err(Error::from(format!("[rbatis-codegen] express trim node error, for example1  trim 'value':  trim start='value': trim start='value',end='value':   error express = {}", trim_express)));
            }
        } else if trim_express.starts_with(ChooseNode::name()) {
            let mut node = ChooseNode { when_nodes: vec![], otherwise_node: None };
            for x in childs {
                match x {
                    NodeType::NWhen(_) => {
