    ...
    Ok(match value.format() {
        MySqlValueFormat::Binary => {
            ...
            if buf.len() == 8 {
                ...
                // We take and truncate to f32 as that's the same behavior as *in*
                // MySQL
...
pub(crate) fn decode_date(value: MySqlValue) -> Result<String, Error> {
    Ok(match value.format() {
        ...
        MySqlValueFormat::Binary => {
            ...
            decode_date_buf(&buf[1..])?.to_string()
        }
    })
}
...
pub(crate) fn decode_date_buf(buf: &[u8]) -> Result<Date, Error> {
    if buf.is_empty() {
        ...
        return Ok(Date { year: 0000, mon: 00, day: 00 });
    }
    Ok(Date { year: LittleEndian::read_u16(buf) as i32, mon: buf[2], day: buf[3] })
}
