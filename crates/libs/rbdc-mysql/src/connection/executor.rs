...
use crate::protocol::statement::Execute as StatementExecute;
use crate::protocol::statement::Prepare;
...
impl MySqlConnection {
    async fn get_or_prepare<'c>(
        ...
        if persistent && self.cache_statement.is_enabled() {
            // in case of the cache being full, close the least recently used
            // statement
            if let Some((id, _)) =
...
impl MySqlConnection {
    ...
    // #[doc(hidden)]
    // pub fn describe<'e, 'q: 'e>(mut self, sql: &'q str) -> BoxFuture<'e,
    // Result<Describe, Error>> {     Box::pin(async move {
    //         self.stream.wait_until_ready().await?;
    //
