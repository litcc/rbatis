...
impl MySqlConnectOptions {
    ...
    ///
    /// By default, the SSL mode is [`Preferred`](MySqlSslMode::Preferred), and the
    /// client will first attempt an SSL connection but fallback to a non-SSL
    ...
    /// let options = MySqlConnectOptions::new().ssl_mode(MySqlSslMode::Required);
    /// ```
    ...
    /// If you need to connect to an older version, we recommend you to change this
    /// to `utf8`.
    ...
    ///
    /// The default collation is derived from the `charset`. Normally, you should
    /// only have to set the `charset`.
    ...
    /// let options = MySqlConnectOptions::new().offset_sec(fastdate::offset_sec());
    /// ```
