...
impl Snowflake {
    ...
    /// from last_timestamp for example1:
    ///
    ///
    /// ```rust
    /// use std::time::UNIX_EPOCH;
...
/// Note:  if you have multiple machines/server, please modify the machine ID and
/// node ID
pub static SNOWFLAKE: LazyLock<Snowflake> =
    LazyLock::new(|| Snowflake::new(1, 1, 0));
