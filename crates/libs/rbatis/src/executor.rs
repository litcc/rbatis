...
impl RBatisConnExecutor {
    pub fn new(id: i64, conn: Box<dyn Connection>, rb: RBatis) -> Self {
        Self { id, conn: Mutex::new(conn), rb }
    }
}
...
/// `RBatisTxExecutor` is a type that represents an executor for transactional
/// operations in RBatis.
///
/// # Type Description
///
/// The `RBatisTxExecutor` is responsible for executing SQL statements within the
/// context of a transaction. It provides methods to execute queries, updates, and
/// other SQL operations, ensuring that all operations are part of the same
...
/// # Type Description
///
/// The `RBatisTxExecutorGuard` implements the `Drop` trait to ensure that
/// transactions are automatically committed or rolled back when the guard goes out
/// of scope. It encapsulates the transaction executor and provides a set of methods
/// to manipulate database transactions.
#[derive(Clone)]
...
impl Debug for RBatisTxExecutorGuard {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RBatisTxExecutorGuard").field("tx", &self.tx).finish()
    }
}
...
impl<'a> TempExecutor<'a> {
    pub fn new(rb: &'a RBatis) -> Self {
        Self { rb, sql: SyncVec::new(), args: SyncVec::new() }
    }
