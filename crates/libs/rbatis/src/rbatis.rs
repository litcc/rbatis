...
impl RBatis {
    ...
    /// init pool.
    /// The default connection pool only binds one type of database driver, please
    /// use separate RBatis for different database drivers DefaultPool is
    /// FastPool,if you want other pool please use init_option
    ...
    /// let _ = rb.init_option::<SqliteDriver, SqliteConnectOptions, DefaultPool>(
    ///     SqliteDriver {},
    ///     opts,
    /// );
    /// ```
    ...
    /// the default name just like `let name =
    /// std::any::type_name::<LogInterceptor>()`
    ///
    /// ```rust
    /// use std::sync::Arc;
    ...
    ///     let table =
    ///         User { id: "".to_string(), name: Some("VARCHAR(50)".to_string()) };
