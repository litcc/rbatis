...
/// Convert a `T` into `rbs::Value` which is an enum that can represent any valid
/// MessagePack data.
///
...
impl ser::Serializer for Value {
    ...
    fn serialize_seq(
        ...
        let se = SerializeVec { vec: Vec::with_capacity(len.unwrap_or(0)) };
        Ok(se)
    }
    ...
    fn serialize_tuple_struct(
        ...
        Ok(SerializeTupleStructVec { name, vec: vec![] })
    }
...
/// Default implementation for tuple variant serialization. It packs given enums as a
/// tuple of an index with a tuple of arguments.
#[doc(hidden)]
