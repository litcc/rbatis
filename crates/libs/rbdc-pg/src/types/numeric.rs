...
pub(crate) enum PgNumeric {
    ...
    ///
    /// A description of these fields can be found here (although the type being
    /// described is the version for in-memory calculations, the field names are
    /// the same): https://github.com/postgres/postgres/blob/bcd1c3630095e48bc3b1eb0fc8e8c8a7c851eba1/src/backend/utils/adt/numeric.c#L224-L269
    Number {
        /// The sign of the value: positive (also set for 0 and -0), or negative.
        sign: PgNumericSign,

        /// The digits of the number in base-10000 with the most significant digit
        /// first (big-endian).
        ///
        /// The length of this vector must not overflow `i16` for the binary
        /// protocol.
        ///
        /// *Note*: the `Encode` implementation will panic if any digit is `>=
        /// 10000`.
        digits: Vec<i16>,

        /// The scaling factor of the number, such that the value will be
        /// interpreted as
        ///
        /// ```text
        ...
        /// How many _decimal_ (base-10) digits following the decimal point to
        /// consider in arithmetic regardless of how many actually follow
        /// the decimal point as determined by `weight`--the comment in the
        /// Postgres code linked above recommends using this only for
        /// ignoring unnecessary trailing zeroes (as trimming nonzero digits means
        /// reducing the precision of the value).
        ///
        /// Must be `>= 0`.
        scale: i16,
    },
...
impl PgNumeric {
    ...
    pub(crate) fn encode(&self, buf: &mut PgArgumentBuffer) {
        match *self {
            PgNumeric::Number { ref digits, sign, scale, weight } => {
