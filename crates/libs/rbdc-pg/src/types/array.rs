    ...
    fn decode(value: PgValue) -> Result<Self, Error> {
        ...
        match format {
            PgValueFormat::Binary => {
                ...
                // appears to have been used in the past to communicate potential
                // NULLS but reading source code back through our
                // supported postgres versions (9.5+) this is never
                // used for anything
            ...
            PgValueFormat::Text => {
                ...
                // NOTE: Nearly *all* types use ',' as the sequence delimiter. Yes,
                // there is one       that does not. The BOX (not
                // PostGIS) type uses ';' as a delimiter.

                // TODO: When we add support for BOX we need to figure out some way
                // to make the       delimiter selection
