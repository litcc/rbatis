...
use crate::types::numeric::PgNumericSign;
...
impl TryFrom<PgNumeric> for BigDecimal {
    ...
    fn try_from(numeric: PgNumeric) -> Result<Self, Error> {
        let (digits, sign, weight) = match numeric {
            PgNumeric::Number { digits, sign, weight, .. } => (digits, sign, weight),
        ...
        if digits.is_empty() {
            // Postgres returns an empty digit array for 0 but BigInt expects at
            // least one zero
            return Ok(0u64.into());
        }
...
impl TryFrom<&'_ BigDecimal> for PgNumeric {
    ...
    fn try_from(decimal: &BigDecimal) -> Result<Self, Error> {
        ...
        // FIXME: is there a way to iterate over the digits to avoid the Vec
        // allocation
        ...
        let weight: i16 = if weight_10 <= 0 {
            ...
            // the `-1` is a fix for an off by 1 error (4 digits should still be 0
            // weight)
            (weight_10 - 1) / 4
        }
        .try_into()
