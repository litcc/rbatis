...
impl PgConnectOptions {
    ...
    /// let options = PgConnectOptions::new().host("localhost");
    /// ```
    ...
    /// ```rust
    /// # use rbdc_pg::options::PgConnectOptions;
    /// let options = PgConnectOptions::new().port(5432);
    /// ```
    ...
    /// ```rust
    /// # use rbdc_pg::options::PgConnectOptions;
    /// let options = PgConnectOptions::new().username("postgres");
    /// ```
    ...
    /// # use rbdc_pg::options::PgConnectOptions;
    /// let options =
    ///     PgConnectOptions::new().username("root").password("safe-and-secure");
    /// ```
    ...
    /// Sets whether or with what priority a secure SSL TCP/IP connection will be
    /// negotiated with the server.
    ///
    /// By default, the SSL mode is disable, and the client will
    /// first attempt an SSL connection but fallback to a non-SSL connection on
    /// failure.
    ///
    ...
    /// let options = PgConnectOptions::new().ssl_mode(PgSslMode::Require);
    /// ```
    ...
    /// ```rust
    /// # use rbdc_pg::options::PgConnectOptions;
    /// let options = PgConnectOptions::new().application_name("my-app");
    /// ```
    ...
    ///
    /// This changes the default precision of floating-point values returned in text
    /// mode (when not using prepared statements such as calling methods of
    /// [`Executor`] directly).
    ///
    /// Historically, Postgres would by default round floating-point values to 6 and
    /// 15 digits for `float4`/`REAL` (`f32`) and `float8`/`DOUBLE` (`f64`),
    /// respectively, which would mean that the returned value may not be exactly
    /// the same as its representation in Postgres.
    ///
    /// The nominal range for this value is `-15` to `3`, where negative values for
    /// this option cause floating-points to be rounded to that many fewer digits
    /// than normal (`-1` causes `float4` to be rounded to 5 digits instead of
    /// six, or 14 instead of 15 for `float8`), positive values cause Postgres to
    /// emit that many extra digits of precision over default (or simply use
    /// maximum precision in Postgres 12 and later), and 0 means keep the default
    /// behavior (or the "old" behavior described above as of Postgres 12).
    ///
    /// SQLx sets this value to 3 by default, which tells Postgres to return
    /// floating-point values at their maximum precision in the hope that the
    /// parsed value will be identical to its counterpart in Postgres. This is
    /// also the default in Postgres 12 and later anyway.
    ///
    /// However, older versions of Postgres and alternative implementations that talk
    /// the Postgres protocol may not support this option, or the full range of
    /// values.
    ///
    /// If you get an error like "unknown option `extra_float_digits`" when
    /// connecting, try setting this to `None` or consult the manual of your
    /// database for the allowed range of values.
    ///
    /// For more information, see:
    /// * [Postgres manual, 20.11.2: Client Connection Defaults; Locale and
    ///   Formatting][20.11.2]
    ...
    /// Set additional startup options for the connection as a list of key-value
    /// pairs.
    ///
    /// # Example
    ///
    ...
    {
        // Do this in here so `options_str` is only set if we have an option to
        // insert
...
fn test_options_formatting() {
    ...
    assert_eq!(options.options, Some("-c geqo=off -c search_path=rbdc".to_string()));
    let options = PgConnectOptions::new()
        .options([("geqo", "off"), ("statement_timeout", "5min")]);
    assert_eq!(
        options.options,
