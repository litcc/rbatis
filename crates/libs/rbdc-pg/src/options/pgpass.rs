...
pub fn load_password(
    ...
    if let Some(file) = custom_file {
        if let Some(password) = load_password_from_file(
            PathBuf::from(file),
            host,
            port,
            username,
    ...
    let default_file =
        dirs::data_dir().map(|path| path.join("postgres").join("pgpass.conf"));
    ...
    while let Ok(n) = reader.read_line(&mut line) {
        ...
        if line.starts_with('#') {
            ...
            if let Some(password) =
                load_password_from_line(&line, host, port, username, database)
            {
                return Some(password);
            }
        }
...
mod tests {
    ...
    use super::find_next_field;
    use super::load_password_from_line;
    use super::load_password_from_reader;
    ...
    fn test_find_next_field() {
        ...
        // : escaped
        test_case("foo\\::bar:baz", Some(Cow::Owned("foo:".to_owned())), "bar:baz");
        // unnecessary escape
        test_case("foo\\a:bar:baz", Some(Cow::Owned("fooa".to_owned())), "bar:baz");
