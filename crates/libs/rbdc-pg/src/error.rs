...
impl PgDatabaseError {
    ...
    /// Indicates an error cursor position as an index into the original query
    /// string; or, a position into an internally generated query.
    #[inline]
    pub fn position(&self) -> Option<PgErrorPosition<'_>> {
        self.0.get_raw(b'P').and_then(atoi).map(PgErrorPosition::Original).or_else(
            || {
...
pub enum PgErrorPosition<'a> {
    ...
    Internal {
        ...
        /// example1, the SQL query issued by a PL/pgSQL function.
        query: &'a str,
    },
}
