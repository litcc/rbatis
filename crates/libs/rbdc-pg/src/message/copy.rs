...
impl Decode<'_> for CopyResponse {
    fn decode_with(mut buf: Bytes, _: ()) -> Result<Self, Error> {
        ...
        Ok(CopyResponse { format, num_columns, format_codes })
    }
}
...
impl Decode<'_> for CopyFail {
    fn decode_with(mut buf: Bytes, _: ()) -> Result<Self, Error> {
        Ok(CopyFail { message: buf.get_str_nul()? })
    }
}
...
impl Decode<'_> for CopyDone {
    fn decode_with(buf: Bytes, _: ()) -> Result<Self, Error> {
        if buf.is_empty() {
            ...
            Err(err_protocol!("expected no data for CopyDone, got: {:?}", buf))
        }
    }
}
