...
impl Decode<'_> for Notice {
    fn decode_with(buf: Bytes, _: ()) -> Result<Self, Error> {
        ...
        // localized S field. Newer versions additionally come with the V
        // field that is guaranteed to be in English. We thus read both
        // versions and prefer the unlocalized one if available.
        ...
        // this enables to keep the access time down for the fields most likely
        // accessed
        ...
        for (field, v) in fields {
            ...
            match field {
                b'S' => {
                    // Discard potential errors, because the message might be
                    // localized
                    severity_s = from_utf8(&buf[v.0 as usize..v.1 as usize])
                        // If the error string is not UTF-8, we have no hope of
                        // interpreting it, localized or not.
                        // The `V` field would likely fail to parse as well.
                        .map_err(|_| notice_protocol_err())?
                        .try_into()
                        // If we couldn't parse the severity here, it might just be
                        // localized.
                        .ok();
                }

                b'V' => {
                    // Propagate errors here, because V is not localized and thus we
                    // are missing a possible variant.
                    severity_v = Some(
