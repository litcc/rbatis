...
pub struct Parse<'a> {
    ...
    /// indication of the number of parameters that might appear in the query
    /// string, only the number that the frontend wants to pre-specify types
    /// for.
...
fn test_encode_parse() {
    ...
    let mut buf = Vec::new();
    let m = Parse { statement: Oid(1), query: "SELECT $1", param_types: &[Oid(25)] };
