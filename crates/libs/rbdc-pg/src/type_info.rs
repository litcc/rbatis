...
pub enum PgType {
    ...
    // A realized user-defined type. When a connection sees a DeclareXX variant it
    // resolves into this one before passing it along to `accepts` or inside of
    // `Value` objects.
    ...
    // NOTE: Do we want to bring back type declaration by ID? It's notoriously
    // fragile but       someone may have a user for it
    DeclareWithOid(Oid),
}
...
impl PgTypeInfo {
    /// Returns the corresponding `PgTypeInfo` if the OID is a built-in type and
    /// recognized by rbdc.
    ...
    ///
    /// Note that the OID for a type is very dependent on the environment. If you
    /// only ever use one database or if this is an unhandled built-in type, you
    /// should be fine. Otherwise, you will be better served using
...
impl PgType {
    /// Returns the corresponding `PgType` if the OID is a built-in type and
    /// recognized by rbdc.
