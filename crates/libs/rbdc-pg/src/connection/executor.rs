...
use crate::message::CommandComplete;
use crate::message::DataRow;
use crate::message::MessageFormat;
use crate::message::ParameterDescription;
use crate::message::Parse;
use crate::message::Query;
use crate::message::RowDescription;
...
async fn prepare(
    ...
    // indicates that the SQL query string is now successfully parsed and has
    // semantic validity
    let _: Nothing = conn.stream.recv_expect(MessageFormat::ParseComplete).await?;
    ...
    let metadata = if let Some(metadata) = metadata {
        ...
        // ensure that if we did fetch custom data, we wait until we are fully ready
        // before continuing
        conn.wait_until_ready().await?;

        Arc::new(PgStatementMetadata { parameters, columns, column_names })
    };
...
async fn recv_desc_params(
    ...
    conn.stream.recv_expect(MessageFormat::ParameterDescription).await
}
...
async fn recv_desc_rows(
    ...
    let rows: Option<RowDescription> = match conn.stream.recv().await? {
        // describes the rows that will be returned when the statement is eventually
        // executed
...
impl PgConnection {
        ...
        while count > 0 {
            match self.stream.recv().await? {
                message if message.format == MessageFormat::PortalSuspended => {
                    ...
                    // this can happen if the last time a statement was used it was
                    // not fully executed such as in [fetch_one]
                }

                message if message.format == MessageFormat::CloseComplete => {
                    // successfully closed the statement (and freed up the server
                    // resources)
                    count -= 1;
                }
    ...
    async fn get_or_prepare<'a>(
        ...
        store_to_cache: bool,
        // optional metadata that was provided by the user, this means they are
    ...
    > {
        ...
        let format = if let Some(mut arguments) = arguments {
            ...
            // apply patches use fetch_optional thaht may produce `PortalSuspended`
            // message, consume messages til `ReadyForQuery` before bind
            // and execute
            self.wait_until_ready().await?;
            ...
            self.stream
                .write(message::Execute { portal: None, limit: limit.into() });

            // finally, [Sync] asks postgres to process the messages that we sent and
            // respond with a [ReadyForQuery] message when it's
            // completely done. Theoretically, we could send
            // dozens of queries before a [Sync] and postgres can handle that.
            // Execution on the server is still serial but it would
            // reduce round-trips. Some kind of builder pattern that is
...
impl PgConnection {
    ...
    pub fn prepare_with<'a>(
        ...
        Box::pin(async move {
            ...
            Ok(PgStatement { sql: sql.to_owned(), metadata })
        })
    }
}
