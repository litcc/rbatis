...
use crate::message::ReadyForQuery;
...
impl PgConnection {
    ...
    async fn recv_ready_for_query(&mut self) -> Result<(), Error> {
        let r: ReadyForQuery =
            self.stream.recv_expect(MessageFormat::ReadyForQuery).await?;
    ...
    /// Queue a simple query (not prepared) to execute the next time this connection
    /// is used.
    ///
...
impl PgConnection {
    fn do_close(&mut self) -> BoxFuture<Result<(), Error>> {
        ...
        // Terminate message and immediately closes the connection.
...
impl Connection for PgConnection {
    ...
    fn ping(&mut self) -> BoxFuture<'_, Result<(), Error>> {
        // By sending a comment we avoid an error if the connection was in the middle
        // of a rowset
