...
use crate::type_info::PgCustomType;
...
impl PgConnection {
    ...
    async fn maybe_fetch_type_info_by_oid(
        ...
        // in the average application, the vast majority of checks should flow
        // through this
        ...
        if should_fetch {
            ...
            self.cache_type_oid.insert(info.0.name().to_string().into(), oid);

            Ok(info)
        } else {
            // we are not in a place that *can* run a query
            // this generally means we are in the middle of another query
            // this _should_ only happen for complex types sent through the TEXT
            // protocol we're open to ideas to correct this.. but it'd
            // probably be more efficient to figure out a way to "prime"
            // the type cache for connections rather than make this
            // fallback work correctly for complex user-defined types for the TEXT
            // protocol
    ...
    fn fetch_type_by_oid(
        ...
        Box::pin(async move {
            ...
            // (String, i8, i8, Oid, Oid, Oid) = query_as(     "SELECT
            // typname, typtype, typcategory, typrelid, typelem, typbasetype FROM
            // pg_catalog.pg_type WHERE oid = $1", )
            //     .bind(oid)
            //     .fetch_one(&mut *self)
            //     .await?;
            ...
            let typ_type = TypType::try_from(
                pg_type.typtype.chars().next().unwrap_or_default() as u8,
            );
            let category = TypCategory::try_from(
                pg_type.typcategory.chars().next().unwrap_or_default() as u8,
            );
            ...
            match (typ_type, category) {
                (Ok(TypType::Domain), _) => {
                    self.fetch_domain_by_oid(oid, typbasetype, pg_type.typname).await
                }
