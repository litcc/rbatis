...
use rbdc::io::BufStream;
use rbdc::io::Decode;
use rbdc::io::Encode;
use rbdc::net::MaybeTlsStream;
use rbdc::net::Socket;

use crate::message::*;
use crate::options::PgConnectOptions;
// the stream is a separate type from the connection to uphold the invariant where an
// instantiated [PgConnection] is a **valid** connection to postgres

// when a new connection is asked for, we work directly on the [PgStream] type until
// the connection is fully established

// in other words, `self` in any PgConnection method is a live connection to postgres
// that is fully prepared to receive queries

pub struct PgStream {
    inner: BufStream<MaybeTlsStream<Socket>>,

    // buffer of unreceived notification messages from `PUBLISH`
    // this is set when creating a PgListener and only written to if that listener
    // is re-used for query execution in-between receiving messages
