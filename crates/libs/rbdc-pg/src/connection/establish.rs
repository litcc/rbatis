...
impl PgConnection {
    pub async fn establish(options: &PgConnectOptions) -> Result<Self, Error> {
        ...
        // its configuration files (such as pg_hba.conf) to determine whether the
            ...
            match message.format {
                MessageFormat::Authentication => match message.decode()? {
                    ...
                    Authentication::Md5Password(body) => {
                        // The frontend must now send a [PasswordMessage] containing
                        // the password (with user name)
                        // encrypted via MD5, then encrypted again
                ...
                MessageFormat::BackendKeyData => {
                    // provides secret-key data that the frontend must save if it
                    // wants to be able to issue cancel requests
                    // later
