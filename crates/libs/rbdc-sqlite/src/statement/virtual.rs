...
use libsqlite3_sys::sqlite3_prepare_v3;
...
use crate::SqliteColumn;
use crate::SqliteError;

// A virtual statement consists of *zero* or more raw SQLite3 statements. We chop up
// a SQL statement on `;` to support multiple statements in one query.
...
pub struct VirtualStatement {
    ...
    /// we use a [`SmallVec`] to optimize for the most likely case of a single
    /// statement
...
impl VirtualStatement {
    ...
    pub(crate) fn prepare_next(
        ...
        self.index =
            self.index.map(|idx| cmp::min(idx + 1, self.handles.len())).or(Some(0));
    ...
    pub fn current(&mut self) -> Option<PreparedStatement<'_>> {
        self.index.filter(|&idx| idx < self.handles.len()).map(move |idx| {
            PreparedStatement {
