...
impl FromStr for SqliteConnectOptions {
    ...
    fn from_str(mut uri: &str) -> Result<Self, Self::Err> {
        ...
        uri = uri.trim_start_matches("sqlite://").trim_start_matches("sqlite:");
        ...
        if let Some(params) = database_and_params.next() {
            for (key, value) in url::form_urlencoded::parse(params.as_bytes()) {
                match &*key {
                    // The mode query parameter determines if the new database is
                    // opened read-only, read-write, read-write
                    // and created if it does not exist, or that the
                    // database is a pure in-memory database that never interacts
                    // with disk, respectively.
                    "mode" => {
                        match &*value {
                            "ro" => {
                    ...
                    // The cache query parameter specifies the cache behaviour across
                    // multiple connections to the same database
                    // within the process. A shared cache is
                    // essential for persisting data across connections to an
                    // in-memory database.
                    "cache" => match &*value {
                        "private" => {
