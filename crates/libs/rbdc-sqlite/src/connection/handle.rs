...
use libsqlite3_sys::sqlite3_close;
...
// [SQLITE_CONFIG_MULTITHREAD] is enabled and [SQLITE_THREADSAFE] was enabled when
// sqlite was compiled. We refuse to work if these conditions are not upheld.
    ...
    pub(crate) fn exec(&mut self, query: impl Into<String>) -> Result<(), Error> {
        ...
        unsafe {
            let status = sqlite3_exec(
                ...
                // callback data
                ptr::null_mut(),
                // out-pointer for the error message, we just use
                // `SqliteError::new()`
                ptr::null_mut(),
            );
...
impl Drop for ConnectionHandle {
    fn drop(&mut self) {
        unsafe {
            ...
            if status != SQLITE_OK {
                // this should *only* happen due to an internal bug in SQLite where
                // we left SQLite handles open
