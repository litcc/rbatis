...
pub(crate) use handle::ConnectionHandleRaw;
...
impl SqliteConnection {
    pub(crate) async fn establish(
        ...
        Ok(Self { worker, row_channel_size: options.row_channel_size })
    }

    /// Returns the underlying sqlite3* connection handle.
    ///
    /// ### Note
    /// There is no synchronization using this method, beware that the background
    /// thread could be making SQLite API calls concurrent to use of this method.
    ///
    /// You probably want to use [`.lock_handle()`][Self::lock_handle] to ensure that
    /// the worker thread is not using the database concurrently.
    #[deprecated(
    ...
    ///
    /// ### Deprecated
    /// Due to the rearchitecting of the SQLite driver, this method cannot actually
    /// function synchronously and return the result directly from
    /// `sqlite3_create_collation_v2()`, so it instead sends a message to the
    /// worker create the collation asynchronously. If an error occurs it will
    /// simply be logged.
    ///
    /// Instead, you should specify the collation during the initial configuration
    /// with [`SqliteConnectOptions::collation()`]. Then, if the collation fails
    /// to apply it will return an error during the connection creation. When
    /// used with a [Pool][crate::pool::Pool], this also ensures that the
    /// collation is applied to all connections automatically.
    ///
    /// Or if necessary, you can call [`.lock_handle()`][Self::lock_handle]
    /// and create the collation directly with
    /// [`LockedSqliteHandle::create_collation()`].
    ///
    /// [`Error::from("WorkerCrashed")`] may still be returned if we could not
    /// communicate with the worker.
    ///
    /// Note that this may also block if the worker command channel is currently
    ...
    /// Lock the SQLite database handle out from the worker thread so direct SQLite
    /// API calls can be made safely.
    ///
...
impl SqliteConnection {
    ...
    pub fn flush(&mut self) -> BoxFuture<'_, Result<(), Error>> {
        ...
        // Well, we could use this to ensure that the command channel has been
        // cleared, but it would only develop a backlog if a lot of queries
        // are executed and then cancelled partway through, and then this
...
impl LockedSqliteHandle<'_> {
    /// Returns the underlying sqlite3* connection handle.
    ///
    /// As long as this `LockedSqliteHandle` exists, it is guaranteed that the
    /// background thread is not making FFI calls on this database handle or any
    /// of its statements.
...
impl Statements {
    fn new(capacity: usize) -> Self {
        Statements { cached: StatementCache::new(capacity), temp: None }
    }
