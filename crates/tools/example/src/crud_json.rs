...
pub async fn main() {
    ...
    let user = User {
        id: Some(1),
        account1: Account { id: Some(2), name: Some("xxx".to_string()) },
        account2: vec![Account { id: Some(2), name: Some("xxx".to_string()) }],
    };
...
async fn create_table(rb: &RBatis) {
    ...
    _ = table_sync::sync(&conn, &SqliteTableMapper {}, to_value!(&table), "user")
        .await;
}
