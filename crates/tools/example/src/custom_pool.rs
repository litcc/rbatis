...
/// define a custom pool(my_pool::DeadPool) use `rb.init_option::<SqliteDriver,
/// SqliteConnectOptions, my_pool::DeadPool>(SqliteDriver {}, opts)`
#[tokio::main]
pub async fn main() {
    _ = fast_log::init(
    ...
    //default_is//let _ = rb.init_option::<SqliteDriver, SqliteConnectOptions,
    // rbatis::DefaultPool>(SqliteDriver{},opts); set custom impl pool
...
mod my_pool {
    ...
    impl Pool for DeadPool {
        ...
        async fn get(&self) -> Result<Box<dyn Connection>, Error> {
            let v =
                self.inner.get().await.map_err(|e| Error::from(e.to_string()))?;
            let conn =
