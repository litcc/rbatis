...
pub async fn main() {
    ...
    rb.intercepts.push(Arc::new(ReadWriteIntercept { read: rb_read }));
...
async fn read_rb() -> RBatis {
    ...
    // "mssql://jdbc:sqlserver://localhost:1433;User=SA;Password={TestPass!123456};
    // Database=master;").unwrap();
    rb.init(rbdc_sqlite::driver::SqliteDriver {}, "sqlite://target/sqlite_read.db")
        .unwrap();
    // table sync done
    sync_table(&rb).await;
    rb
...
async fn write_rb() -> RBatis {
    ...
    rb.init(rbdc_sqlite::driver::SqliteDriver {}, "sqlite://target/sqlite_write.db")
        .unwrap();
    // table sync done
    sync_table(&rb).await;
    rb
