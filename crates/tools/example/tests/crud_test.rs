...
mod test {
    ...
    fn test_query_decode() {
        let f = async move {
            ...
            let r: Vec<MockTable> =
                rb.query_decode("select * from mock_table", vec![]).await.unwrap();
            let mut conn = rb.acquire().await.unwrap();
            let r: Vec<MockTable> =
                conn.query_decode("select * from mock_table", vec![]).await.unwrap();
        };
        block_on(f);
    }
    ...
    fn test_update_by_column() {
        let f = async move {
            ...
            let r = MockTable::update_by_column(&rb, &t, "id").await.unwrap();
    ...
    fn test_update_by_column_skip_null() {
        let f = async move {
            ...
            let r = MockTable::update_by_column_skip(&rb, &t, "id", false)
                .await
                .unwrap();
    ...
    fn test_update_by_column_batch() {
        ...
        let f = async move {
            ...
            let r = MockTable::update_by_column_batch(&rb, &tables, "id", 2)
                .await
                .unwrap();
        };
        block_on(f);
    ...
    fn test_select_all_by_id() {
        let f = async move {
            ...
            rb.set_intercepts(vec![Arc::new(MockIntercept::new(queue.clone()))]);
            rb.init(MockDriver {}, "test").unwrap();
            let r = MockTable::select_all_by_id(&rb, "1", "1").await.unwrap();
            let (sql, args) = queue.pop().unwrap();
            println!("{}", sql);
    ...
    fn test_select_by_dto() {
        let f = async move {
            ...
            rb.set_intercepts(vec![Arc::new(MockIntercept::new(queue.clone()))]);
            rb.init(MockDriver {}, "test").unwrap();
            let r = MockTable::select_by_dto(&rb, DTO { id: "1".to_string() })
                .await
                .unwrap();
            let (sql, args) = queue.pop().unwrap();
            println!("{}", sql);
    ...
    fn test_update_by_name() {
        let f = async move {
            ...
            let r = MockTable::update_by_name(&rb, &t, "test").await.unwrap();
            let (sql, args) = queue.pop().unwrap();
            println!("{}", sql);
    ...
    fn test_update_by_dto() {
        let f = async move {
            ...
            let r = MockTable::update_by_dto(&rb, &t, DTO { id: "2".to_string() })
                .await
                .unwrap();
            let (sql, args) = queue.pop().unwrap();
            println!("{}", sql);
    ...
    fn test_select_by_column() {
        let f = async move {
            ...
            rb.set_intercepts(vec![Arc::new(MockIntercept::new(queue.clone()))]);
            rb.init(MockDriver {}, "test").unwrap();
            let r = MockTable::select_by_column(&rb, "id", "1").await.unwrap();
            let (sql, args) = queue.pop().unwrap();
            println!("{}", sql);
    ...
    fn test_select_in_column() {
        let f = async move {
            ...
            rb.init(MockDriver {}, "test").unwrap();
            let r =
                MockTable::select_in_column(&rb, "1", &["1", "2"]).await.unwrap();
            let (sql, args) = queue.pop().unwrap();
            println!("{}", sql);
    ...
    fn test_delete_in_column() {
        let f = async move {
            ...
            rb.init(MockDriver {}, "test").unwrap();
            let r =
                MockTable::delete_in_column(&rb, "1", &["1", "2"]).await.unwrap();
            let (sql, args) = queue.pop().unwrap();
            println!("{}", sql);
            assert_eq!(sql, "delete from mock_table where 1 in (?,?)");
            assert_eq!(args, vec![to_value!("1"), to_value!("2")]);
    ...
    fn test_tx() {
        let f = async move {
            ...
            let mut tx =
                rb.acquire_begin().await.unwrap().defer_async(|_tx| async {});
            let _r = MockTable::insert(&tx, &t).await.unwrap();
        };
        block_on(f);
    }
    ...
    fn test_pysql_select_page() {
        let f = async move {
            ...
            let r = pysql_select_page(
                &rb,
                &PageRequest::new(1, 10),
                PySqlSelectPageArg { name: "aaa".to_string() },
                "test",
            )
            .await
            .unwrap();
